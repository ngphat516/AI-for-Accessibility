...
const SidebarNotes: React.FC<SidebarNotesProps> = ({ notes, onSaveNote, onDeleteNote, isFocused, focusedIndex }) => {
  ...
  const activeIdx = isFocused && !editId && filteredNotes.length > 0 ? (focusedIndex ?? 0) % filteredNotes.length : -1;
  ...
  const handleStartCreate = () => {
    setFormData({ title: '', chatContext: 'THUỘC VỀ ĐOẠN CHAT NÀO', summary: '', createdAt: new Date().toLocaleDateString('vi-VN') });
    setEditId('new');
  };
  ...
  return (
    <div className={`h-full p-6 flex flex-col gap-5 transition-colors duration-300 ${isFocused ? 'bg-[#C8C8C8]' : 'bg-[#D6D6D6]'}`}>
      <div className="space-y-4">
        <p className="text-[11px] font-black text-gray-500 uppercase tracking-[0.3em] ml-2">GHI CHÚ</p>
        ...
        <div className="flex gap-2">
          <button className="bg-[#BDBDBD] px-6 py-2 rounded-full text-[10px] font-bold uppercase hover:bg-gray-400 transition-all shadow-sm">LỌC</button>
          {!editId && (
            <button 
      ...
      <div className="flex-1 overflow-y-auto space-y-6 pr-2 custom-scrollbar">
        {editId && (
          <div className="bg-[#AFAFAF] rounded-[40px] p-8 border-4 border-gray-800 shadow-2xl animate-in zoom-in-95 duration-200">
             ...
             />
             <p className="text-[10px] uppercase font-bold text-gray-500 mb-4">{formData.createdAt}</p>
             ...
             <div className="flex gap-3 mt-6">
               <button onClick={handleSave} className="flex-1 bg-gray-800 text-white text-[10px] font-bold py-3 rounded-full uppercase shadow-lg active:scale-95">XÁC NHẬN</button>
               <button onClick={() => setEditId(null)} className="flex-1 bg-white/20 border border-gray-600 text-[10px] font-bold py-3 rounded-full uppercase active:scale-95">HỦY</button>
             </div>
          </div>
        )}
        ...
        {filteredNotes.length === 0 && !editId ? (
          <div className="text-center py-24 opacity-20">
            <p className="text-[11px] uppercase font-black tracking-widest">Trống</p>
          </div>
        ) : (
            ...
            editId !== note.id && (
              ...
              >
                <div className="flex justify-between items-start mb-1">
                  <h3 className="text-xs font-black uppercase line-clamp-1 flex-1">{note.title}</h3>
                  <div className="flex gap-4 ml-4 shrink-0 transition-opacity">
                    <button onClick={() => handleStartEdit(note)} className="text-[10px] font-black uppercase text-gray-700 hover:text-black hover:underline underline-offset-4">CHỈNH SỬA</button>
                    <button onClick={() => onDeleteNote(note.id)} className="text-[10px] font-black uppercase text-gray-500 hover:text-red-700">XÓA</button>
                  </div>
                </div>
                ...
                <button className="bg-[#D8D8D8] px-8 py-1.5 rounded-full text-[10px] font-black uppercase mb-6 shadow-sm border border-black/5 hover:bg-white transition-all">LINK</button>
                ...
                <div className="space-y-3">
                  {note.summary.split('\n').filter(line => line.trim()).map((line, lIdx) => (
                    <div key={lIdx} className="flex gap-3 items-start">
                      <span className="text-[14px] mt-[-3px] text-gray-900">•</span>
                      <p className="text-[11px] uppercase font-bold leading-tight text-gray-800">{line.replace(/^•\s*/, '')}</p>
                    </div>
                  ))}
                </div>
              </div>
